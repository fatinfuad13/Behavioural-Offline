...
public class Course {
    ...
    // Print transition message for admin/manual changes
    // Do NOT print if this is automatic FULL update or CANCELLED from enrollment logic
    boolean isAutoFull = (oldStatus == CourseStatus.OPEN && newStatus == CourseStatus.FULL);
    boolean isAutoCancel = (newStatus == CourseStatus.CANCELLED && oldStatus != CourseStatus.CANCELLED);

    if (!isAutoFull && !isAutoCancel && oldStatus != newStatus) {
        System.out.println(code + " transitioned " + oldStatus + " -> " + newStatus);
    }
    ...
    //RegistrarMediator.getInstance().changeCourseCapacity(this, newCapacity);
    if (newCapacity < 0) newCapacity = 0;
        System.out.println("Setting capacity of " + code + " to " + newCapacity);
        this.capacity = newCapacity;
        state.setCapacity(newCapacity);
